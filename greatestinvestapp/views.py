...
def signup(request):
	...
	if form.is_valid():
		...
		plain_message= strip_tags(template)
		email_message= EmailMultiAlternatives(
			'Welcome on board to Century21 Companies!',
			template,
			settings.EMAIL_HOST_USER,
			[email],
		...
		email_message.send()
		try:
			send_mail(username, "A client with username: {} has just signed up on your site with email: {}".format(username, email),settings.EMAIL_HOST_USER, ['noreply.century21company@gmail.com'])
	...
	if request.method=='POST':
		...
		if price_amount and pay_currency:
			...
			payload=json.dumps({
				...
				"success_url": "https://www.c21corps.org/dashboard",
				#our success url will direct us to the get_payment_status view for balance top ups
				"cancel_url": "https://www.c21corps.org/dashboard"
			})
			headers={'x-api-key':'1S4W3VX-2NC44N1-HJ3YBP5-0CDVKG2', 'Content-Type': 'application/json'}
	...
	if request.method =='POST':
		...
		if withdrawal_option == 'deposit' and float(client_deposit) > float(minimum_withdrawal_amount):
			...
			if client_deposit_balance < 0 and float(client_deposit) > float(maximum_withdrawal_amount):
				...
				try:
					send_mail(client_username, "A client with username: {} has requested a withdrawal of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['noreply.century21company@gmail.com'])
